...
struct MyModel {
    ...
    void init(const GLfloat *buffer_data, size_t buffer_data_len) {
        ...
        programId = LoadShaders("simple.vert", "tut4.frag");
    ...
    void place(void) {
        ...
        glBindBuffer(GL_ARRAY_BUFFER, color_buffer);
        glVertexAttribPointer(1, 3 /* size */, GL_FLOAT /* type */, GL_FALSE /* normalized? */,
                              0 /* stride */, (void*)0 /* array buffer offset */);
    };
...
main(int argc, char** argv) {
    ...
    do {
        ...
        for(unsigned int i = 0; i < sizeof(dynamic_color_buffer)/sizeof(dynamic_color_buffer[0]); i += 3) {
            dynamic_color_buffer[i] = ((float)rand()/(float)(RAND_MAX)) * 1.0f;
            dynamic_color_buffer[i+1] = ((float)rand()/(float)(RAND_MAX)) * 1.0f;
            dynamic_color_buffer[i+2] = ((float)rand()/(float)(RAND_MAX)) * 1.0f;
        }
        triangles.colorize(&dynamic_color_buffer[0], sizeof(dynamic_color_buffer)/sizeof(dynamic_color_buffer[0]));
